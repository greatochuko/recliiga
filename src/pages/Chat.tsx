...
interface Chat {
  ...
  initials: string
  image?: string
  members?: Array<{ name: string; initials: string; image: string }>
  unreadCount: number
  role?: string
}
...
function ChatContent() {
  ...
  const [chats, setChats] = useState<Chat[]>([
    ...
    { id: 'john-smith', name: "John Smith", type: 'individual', role: 'player', lastMessage: "When's our next game?", time: "10:03 PM", initials: "JS", unreadCount: 1 },
    { id: 'emma-johnson', name: "Emma Johnson", type: 'individual', role: 'organizer', lastMessage: "Can you sub in for tomorrow's match?", time: "10:01 PM", initials: "EJ", unreadCount: 2 },
    { id: 'mike-brown', name: "Mike Brown", type: 'individual', role: 'player', lastMessage: "Team meeting at 7 PM tonight.", time: "9:01 PM", initials: "MB", unreadCount: 0 },
    { id: 'sarah-davis', name: "Sarah Davis", type: 'individual', role: 'player', lastMessage: "Great game yesterday!", time: "Friday", initials: "SD", unreadCount: 0 },
    { id: 'chris-wilson', name: "Chris Wilson", type: 'individual', role: 'player', lastMessage: "Practice schedule for next week?", time: "Thursday", initials: "CW", unreadCount: 0 },
  ])
  ...
  const individualMessages = [
    { name: selectedChat?.name, message: "Hey coach, I wanted to discuss our strategy for the upcoming game. Do you have any specific plays in mind?", time: "8:55 PM", initials: selectedChat?.initials },
    ...
    { name: selectedChat?.name, message: "Sounds great! Should we focus on our passing game or running plays?", time: "9:01 PM", initials: selectedChat?.initials },
    ...
    { name: selectedChat?.name, message: "Agreed. I'll let the team know to be prepared for some intensive passing drills tomorrow.", time: "9:10 PM", initials: selectedChat?.initials },
  ]
              ...
              <div className="relative">
                <Avatar>
                  {chat.image ? (
                    ...
                    <AvatarFallback>{chat.type === 'group' ? <Users className="h-6 w-6 text-[#FF7A00]" /> : chat.initials}</AvatarFallback>
                  )}
                </Avatar>
                {chat.unreadCount > 0 && (
                  <div className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs bg-[#FF7A00] text-white rounded-full">
                    {chat.unreadCount}
                  </div>
                )}
              </div>
        ...
        {selectedChat && (
          <>
              ...
              <div className="flex items-center">
                <Avatar className="w-10 h-10 mr-3">
                  {selectedChat.image ? (
                    ...
                    <AvatarFallback>{selectedChat.type === 'group' ? <Users className="h-6 w-6 text-[#FF7A00]" /> : selectedChat.initials}</AvatarFallback>
                  )}
                </Avatar>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {(selectedChat.type === 'group' ? groupMessages : individualMessages).map((message, index) => (
