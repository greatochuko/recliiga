      ...
      <div className="flex h-[calc(100vh-180px)] border rounded-lg overflow-hidden">
        ...
        <div className="w-80 border-r border-gray-200 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {chats.map(chat => <div key={chat.id} className={`flex items-center p-4 space-x-3 cursor-pointer relative ${chat.id === activeConversation?.id ? 'bg-gray-100 before:absolute before:left-0 before:top-0 before:h-full before:w-1 before:bg-[#FF7A00]' : 'hover:bg-gray-50'}`} onClick={() => handleChatSelect(chat)}>
                <div className="relative">
                  <Avatar>
                    <AvatarImage src={chat.avatar} alt={chat.name} />
                    <AvatarFallback>{chat.type === 'group' ? <Users className="h-6 w-6 text-[#FF7A00]" /> : getInitials(chat.name)}</AvatarFallback>
                  </Avatar>
                  {chat.unread > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs bg-[#FF7A00]" aria-label={`${chat.unread} unread messages`}>
                      {chat.unread}
                    </Badge>}
                </div>
        ...
        <section className="flex-1 flex flex-col">
          {activeConversation && <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {getMessageData(activeConversation).map((message, index) => <div key={index} className={`flex items-start space-x-2 ${message.name === "John Doe" ? "justify-end" : ""}`}>
              ...
              <div className="border-t border-gray-200 p-4">
                <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
                  <Input type="text" placeholder="Type your message..." value={messageInput} onChange={e => setMessageInput(e.target.value)} className="flex-1 border-[#707B81] focus:border-[#FF7A00] focus:ring-[#FF7A00]" />
        ...
        {/* Profile sidebar */}
        {isProfileVisible && activeConversation && <aside className="w-64 border-l border-gray-200 p-4">
            <div className="flex flex-col items-center">
              ...
              {activeConversation.type === 'group' && activeConversation.members && <div className="w-full mt-4">
                  <h5 className="text-sm font-medium text-[#707B81] mb-2">Members</h5>
                  <div className="max-h-[300px] overflow-y-auto">
                    {activeConversation.members.map((member, index) => <div key={index} className="flex items-center space-x-2 mb-2">
