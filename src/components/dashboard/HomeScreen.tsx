...
const LeagueSelector = ({
  ...
  return <div className="space-y-4">
      ...
      <Select defaultValue={leagues[0].id} onValueChange={onLeagueChange}>
        <SelectTrigger className="w-[180px]">
          <SelectValue placeholder="Select League" />
        </SelectTrigger>
        <SelectContent>
          {leagues.map(league => <SelectItem key={league.id} value={league.id}>
              {league.name}
            </SelectItem>)}
        </SelectContent>
      </Select>
...
const TeammatesToRate = () => {
  ...
  return <div className="flex flex-col h-full">
      <div className="flex justify-between items-center mb-4">
        ...
        <Button variant="link" className="text-[#FF7A00] hover:text-[#FF7A00]/90" onClick={() => navigate('/rate-teammates')}>
          View all
        </Button>
      </div>
      <div className="grid grid-cols-2 gap-4 flex-grow">
        {mockTeammates.map(teammate => <div key={teammate.id} className="bg-white rounded-lg p-3 shadow-sm border border-gray-100 flex items-center justify-between cursor-pointer hover:bg-gray-50" onClick={handleTeammateClick}>
  ...
  return <div className="space-y-8 p-4 md:p-6">
      ...
      <section className="mb-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button variant="link" className="text-[#FF7A00] hover:text-[#FF7A00]/90" onClick={() => navigate('/events')}>
            View all
          </Button>
        </div>
        <div className="space-y-4">
          {mockEvents.map(event => <EventCard key={event.id} event={event} showLeagueName={true} />)}
        </div>
      </section>
    </div>;
}
