...
export const fetchEvents = async (): Promise<Event[]> => {
  try {
    ...
    return events.map(event => {
      ...
      if (userId) {
        const userRsvp = event.event_rsvps?.find(rsvp => rsvp.player_id === userId);
        if (userRsvp) {
      ...
      return {
        ...
        status,
        spotsLeft,
        draftStatus: event.draft_status as 'not_started' | 'in_progress' | 'completed'
      };
    });
  } catch (error) {
 ...
 */
export const fetchEventById = async (eventId: string | number): Promise<Event | null> => {
  try {
